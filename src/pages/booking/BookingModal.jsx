 ...
 console.log(props.isOpen);
  ...
  useEffect(() => {
    if (!props.propertyId) return;
        axios.get(`http://localhost:8080/api/v1/stayeasy/booking/listing/${props.propertyId}`)
            .then(response => {
                if (response) {
    ...
    const bookedDates = place.map(booking => {
      let disabledDates = [];
  ...
  useEffect(() => {
    ...
    const newURL = `http://localhost:3000/booking/${props.propertyId}?${searchParams.toString()}`;
      ...
      <div className="fixed z-10 inset-0 overflow-y-auto pb-20">
        <div className="flex items-end justify-center min-h-screen  pb-20 text-center sm:block sm:p-0">
          ...
          <div className="inline-block align-bottom bg-white rounded-lg  text-left overflow-hidden shadow-xl transform transition-all  sm:p-6">
            ...
            color={(date) => bookedDates.some(booking => date >= booking.startDate && date <= booking.endDate) ? '#020617' : null}
          />
