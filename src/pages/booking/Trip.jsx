    ...
    axios
      ...
      .then((response) => {
        ...
        const convertedData = filteredData.map((cardData, index) => {
          ...
          const checkoutTime = cardData.checkOut.time
            ? cardData.checkOut.time
            : "12:00";
          const checkinTime = cardData.checkIn.time
            ? cardData.checkIn.time
            : "07:00";
          const checkoutDayOfWeek = [
            "Chủ nhật",
            "Thứ hai",
            "Thứ ba",
            "Thứ tư",
          ...
          return {
            ...
            guestName:
              cardData.userDTOS.lastName + "  " + cardData.userDTOS.firstName,
            avatar: cardData.userDTOS.avatar
              ? cardData.userDTOS.avatar
              : "https://cdn-icons-png.flaticon.com/512/5968/5968596.png",
  ...
  return (
    <>
      ...
      <div className="mt-32 py-16 max-[768px]:mt-10 min-[768px]:px-[12rem] max-[768px]:px-[6rem] max-[425px]:px-[3rem]">
          ...
          {dataCard.map((e, i) => {
            return (
              <>
                <button
                  onClick={() => {
                    openDetailTrip(e.index);
                  }}
                  key={i}
                  className="w-[31.8%] max-[1062px]:w-[100%] h-full shadow-lg rounded-xl"
                >
                  <div className="relative bg-white">
                    ...
                    <div className="absolute top-2 left-2 p-2 bg-white text-xl rounded-lg">
                      {e.checkout.day - e.checkin.day >= 7
                        ? Math.round((e.checkout.day - e.checkin.day) / 7)
                        : e.checkout.day - e.checkin.day}
                      {e.checkout.day - e.checkin.day >= 7 ? " Tuần" : " Ngày"}
                    </div>
                  </div>
        ...
        <p>
          không thể tìm thấy đặt phòng của bạn ở đây?{" "}
          <span className="font-medium underline"> Đến trung tâm hỗ trợ</span>
        </p>
      </div>
      <Footer />
    </>
...
const TripDetail = ({ userId, data, toggleClose }) => {
  ...
  const checkIsOwner = userId === data.userId;
  ...
  const handCheckout = (id) => {
    ...
    axios
      .post(`http://localhost:8080/api/v1/stayeasy/payment/performPayout/${id}`)
      .then((response) => {
        setLoad(true);
      ...
      .catch((error) => {
        ...
        Alert(
          2000,
          "Trả phòng trong ngày",
          "Chúng tôi đã ghi nhận thông tin của bạn và đang chờ xử lí",
          "warning",
          "OK"
        );
      });
  ...
  const handleCancel = (data, status) => {
    ...
    const checkDateCancel = differenceInCalendarDays(
    ...
    if (checkDateCancel >= 5) {
      ...
      setMessage(
        "Hoàn trả 100% cho booking của bạn, khấu trừ 50% phí dịch vụ ariBnb"
      );
    } else if (checkDateCancel <= 2 && checkDateCancel > 1) {
      refundPercentage =
        (data.total * 70) / 100 - data.total * (data.bnbFee / 200); // Hoàn 50% // Hoàn 70%
      ...
      setMessage(
        "Nếu bạn hủy vé, chúng tôi chỉ có thể gửi lại bạn phí dọn dẹp (nếu có)"
      );
    }
  ...
  const submitCancel = (data) => {
    const refundDTO = {
      ...
      noteToPayer: generateNoteToPayer()
        ? generateNoteToPayer()
        : "Hủy booking",
    ...
    try {
      ...
      if (response.data.message === "Hủy booking thành công và đã hoàn tiển.") {
        ...
        Alert(
          2000,
          "Hủy đặt phòng",
          "Hủy booking thành công và đã hoàn tiển",
          "succsess",
          "OK"
        );
        window.location.reload();
  ...
  const goToNext = () => {
    setCurrentIndex((prevIndex) =>
      Math.min(prevIndex + itemsToMove, data.img.length - itemsPerSlide + 1)
    );
  };
  if (load) {
    return (
      <div className="flex justify-center items-center h-screen w-screen bg-gray-100 opacity-100">
        <div className="flex flex-col items-center">
          <img
            src="https://cdn.dribbble.com/users/2356828/screenshots/15188850/media/f8152a69b40f21eec559e6e0d05a46f1.gif"
            alt="Loading"
            className="w-20% h-20%"
          />
          <p className="text-lg font-medium text-gray-700 mt-4">
            Quá trình thanh toán sắp hoàn tất...
          </p>
        </div>
      </div>
    );
  ...
  return (
    <>
      ...
      <div className="fixed top-0 inset-0 flex items-center justify-center z-[1000]">
        ...
        <div
          className="absolute inset-0 bg-gray-500 opacity-50 m-0 p-0"
          onClick={toggleClose}
        ></div>
        <div className="mx-[25rem] bg-white overflow-scroll rounded-xl z-[1000] h-[65rem]">
          ...
          <div className="relative overflow-hidden">
            ...
            <div
              ...
              style={{
                width: `${100 * (data.img.length / itemsPerSlide)}%`,
                transform: `translateX(-${
                  (100 / data.img.length) * currentIndex
                }%)`,
              }}
            >
              {data.img.map((image, index) => (
                <button className="text-start w-full" key={index}>
                  <img
                    src={image}
                    alt="Header"
                    className="w-full h-[35rem] object-cover rounded-b-2xl"
                  />
                </button>
              ))}
            </div>
            ...
            <div className="absolute top-5 right-10">
              <button
                onClick={toggleClose}
                className={`flex items-center justify-center w-[2.5rem] h-[2.5rem] bg-white rounded-full text-black cursor-pointer`}
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  height="14"
            ...
            <div className="absolute top-[50%] left-10">
              <button
                ...
                className={`flex items-center justify-center w-[3.1rem] h-[3.1rem] bg-gray-500 rounded-full text-black cursor-pointer ${
            ...
            <div className="w-[60%] pr-24">
              ...
              <div className="flex items-center gap-6 mt-3">
                <svg
                  ...
                  role="presentation"
                  focusable="false"
                  className="block h-[24px] w-[24px] mb-3"
                  fill="current"
                >
              ...
              <div className="flex gap-10 justify-evenly items-center">
                <div>
                  ...
                  <p className="font-medium text-3xl">
                    {data.checkin.date}, {data.checkin.day}/{data.checkin.month}
                  </p>
                ...
                <div>
                  ...
                  <p className="font-medium text-3xl">
                    {data.checkout.date}, {data.checkout.day}/
                    {data.checkout.month}
                  </p>
                  ...
                  {data.comfirm === "PENDING" && data.status ? (
                    <div className="flex gap-4 items-center">
                      {!showCancellationCard && (
                        <div className="flex gap-4 items-center">
                          <p className="bg-gray-400 shadow-xl py-3 px-5 rounded-xl text-3xl font-medium">
                            Chờ Xác Nhận
                          </p>
                          <button
                            className="bg-zinc-950 text-white shadow-xl py-3 px-5 rounded-xl text-3xl font-medium"
                            onClick={() => handleCancel(data, true)}
                          >
                            Hủy
                          </button>
                        </div>
                        ...
                        <div className="mt-5 p-4 bg-white shadow-xl flex flex-col gap-4">
                          ...
                          <div className="flex flex-col gap-2 mb-4">
                            <label>
                              <input
                                type="checkbox"
                                name="cannotArrive"
                                checked={cancellationReasons.cannotArrive}
                            ...
                            <label>
                              <input
                                type="checkbox"
                                name="changePlans"
                                checked={cancellationReasons.changePlans}
                                onChange={handleCheckboxChange}
                                className="form-checkbox h-5 w-5"
                              />{" "}
                              Thay đổi kế hoạch
                          ...
                          <div className="flex flex-col space-y-4">
                            <p className="text-3xl font-semibold">
                              Khoản tiền hoàn trả: {refundAmount} $
                            </p>
                            <p
                            ...
                            {(data.total - refundAmount).toFixed(2) > 0 && (
                              <div>
                                <p className="text-2xl font-semibold text-red-600">
                                  Khoản khấu trừ: -{" "}
                                  {(data.total - refundAmount).toFixed(2)} $
                                </p>
                              </div>
                            )}
                          </div>
                          ...
                          <div className="flex justify-end gap-4">
                            <button
                              className="bg-zinc-950 hover:bg-zinc-950 text-white font-bold py-2 px-4 rounded"
                              onClick={() => submitCancel(data)}
                            >
                              Xác nhận
                            </button>
                            <button
                              className="bg-pink-600 hover:bg-gray-400 text-white font-bold py-2 px-4 rounded"
                    ...
                    <div className="flex gap-4 items-center">
                      {!showCancellationCard && (
                        <div className="flex gap-4 items-center">
                          <p className="bg-pink-600 shadow-xl py-3 px-5 rounded-xl text-3xl font-medium">
                            Đã Xác Nhận
                          </p>
                          <button
                            className="bg-zinc-950 text-white  shadow-xl py-3 px-5 rounded-xl text-3xl font-medium"
                    ...
                    <a href={`/explore`}>
                      <p className="bg-red-600 shadow-xl py-3 px-5 rounded-xl text-3xl font-medium">
                        ĐẶT LẠI
                      </p>
                    </a>
                  ) : data.comfirm === "COMPLETED" ? (
                    <p className="bg-green-600 shadow-xl py-3 px-5 rounded-xl text-3xl font-medium">
                      ĐÃ HOÀN THÀNH CHUYẾN ĐI
                    </p>
                  ) : data.comfirm === "IN_PROGRESS" ? (
                    <div className="flex flex-col gap-2 mb-4">
                      <p className="bg-lime-400  shadow-xl py-3 px-5 rounded-xl text-3xl font-medium">
                        ĐANG DIỄN RA
                      </p>
                      <button
                        className="bg-zinc-950 text-white  shadow-xl py-3 px-5 rounded-xl text-3xl font-medium"
                        onClick={() => handCheckout(data.idBooking)}
                      >
                        Trả Phòng
                      </button>
                    </div>
          ...
          {/* session 2 */}
          {!(data.comfirm === "REJECTED" || data.confirm === "COMPLETED") && (
            <div className="py-5 px-10">
              ...
              <div className="flex gap-4 items-center mt-5">
                ...
                <div className="mt-3">
                  <h3>Ai sẽ đến</h3>
                  <h4>
                    {data.guestName} cùng {data.numGuest} khách
                  </h4>
                </div>
              </div>
              <hr />
              ...
              <div className="flex gap-4 items-center mt-5">
                <div className="mt-3">
                  <h3>Mã đặt phòng</h3>
                  <p className="bg-gray-100 px-4">
                    {data.idBooking
                      .substring(data.idBooking.length - 5)
                      .toUpperCase()}
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-4 items-center mt-5">
                <div className="mt-3">
                  ...
                  <p className="">
                    Hoàn 100% trước ngày {data.checkin.day - 5} tháng{" "}
                    {data.checkin.month}.
                  </p>
                  <p className="">
                    Hoàn 50% phí dịch vụ airbnb trước ngày{" "}
                    {data.checkin.day - 3} tháng {data.checkin.month}, đã bao
                    gồm thuế.
                  </p>
                  <p className="">
                    Hoàn 70% booking phòng trước ngày {data.checkin.day - 2}{" "}
                    tháng {data.checkin.month}, đã bao gồm thuế.
                  </p>
                  <p className="">
                    Chỉ hoàn phí lau dọn (nếu có) trước ngày{" "}
                    {data.checkin.day - 1} tháng {data.checkin.month}.
                  </p>
                  <Link>
              ...
              <div className="flex gap-4 items-center mt-5">
                <p>
                  Booking cuả bạn được bảo đảm bởi{" "}
                  <span className="text-[#FF385C] font-medium">air</span>
                  <span className="text-black font-medium">cover </span>
          ...
          <div className="py-5 px-10">
            ...
            <div className="flex gap-4 items-center mt-5">
              <div className="mt-3">
                ...
                <p>
                  Bạn sẽ tìm thấy hướng dẫn để đến đây 48 giờ trước khi nhận
                  phòng
                </p>
              </div>
            </div>
          ...
          <div className="py-5 px-10">
            ...
            <p>
              If you need help, we're availiable 24/7 from anywhere in the world
            </p>
...
const IconButton = ({ icon, title, toogleBtn }) => {
  return (
    <button
      onClick={toogleBtn}
      className="flex justify-between items-center w-full pr-14"
    >
